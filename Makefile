...
build-worker: build-enigma-common
	cd worker; docker build --build-arg CORE_IMAGE=enigmampc/core-artifact-${ext}:${DOCKER_TAG} --build-arg -f worker.Dockerfile -t enigmampc/worker_${ext}:${DOCKER_TAG} .

.PHONY: build-core
build-core:
	cd worker; docker build --build-arg DEBUG=${DEBUG} --build-arg SGX_MODE=${SGX_MODE} -f compile_core.Dockerfile -t enigmampc/core-artifact-${ext}:${DOCKER_TAG} .
