...
describe('Enigma tests', () => {
    ...
    it('should execute compute task', async () => {
        ...
        task1 = new Promise((resolve, reject) => {
            enigma.computeTask(taskFn, taskArgs1, taskGasLimit, taskGasPx, accounts[0], factorAddr)
                .on(eeConstants.SEND_TASK_INPUT_RESULT, (result) => resolve(result))
                .on(eeConstants.ERROR, (error) => reject(error));
        });
        task2 = new Promise((resolve, reject) => {
            enigma.computeTask(taskFn, taskArgs2, taskGasLimit, taskGasPx, accounts[0], factorAddr)
                .on(eeConstants.SEND_TASK_INPUT_RESULT, (result) => resolve(result))
                .on(eeConstants.ERROR, (error) => reject(error));
        });
        task1 = await task1;
    ...
    it('should get the confirmed tasks', async () => {
        ...
        } while (task1.ethStatus != 3 || task2.ethStatus != 2);
