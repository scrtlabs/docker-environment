...
describe('Enigma tests', () => {
    ...
    it('should execute compute task', async () => {
        let taskFn1 = 'find_number_of_prime_factors(uint64)';
        let taskArgs1 = [[8972, 'uint64']];
        let taskFn2 = 'mul(uint256,uint256)';
        let taskArgs2 = [
            [76, 'uint256'],
            [17, 'uint256'],
        ];
        ...
        task1 = new Promise((resolve, reject) => {
            enigma1.computeTask(taskFn1, taskArgs1, taskGasLimit, taskGasPx, accounts[0], factorAddr,1, 'user1')
                .on(eeConstants.SEND_TASK_INPUT_RESULT, (result) => resolve(result))
                .on(eeConstants.ERROR, (error) => reject(error));
        });

        task2 = new Promise((resolve, reject) => {
            enigma2.computeTask(taskFn2, taskArgs2, taskGasLimit, taskGasPx, accounts[0], calculatorAddr, 1, 'user2')
                .on(eeConstants.SEND_TASK_INPUT_RESULT, (result) => resolve(result))
                .on(eeConstants.ERROR, (error) => reject(error));
        });
        task1 = await task1;
